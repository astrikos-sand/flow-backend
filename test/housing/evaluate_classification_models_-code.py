def func(models, X_test, y_test, weights):
        ...
        if weights:
            combined_score = sum(
                [
                    ...
                    for weight, metric in zip(
                        weights.values(), [accuracy, precision, recall, f1]
                    )
                ]
            )
        else:
            ...
            combined_score = sum(
                [
                    weight * metric
                    for weight, metric in zip(
                        default_weights.values(), [accuracy, precision, recall, f1]
                    )
                ]
            )

        evaluation_metrics[f"Model_{i+1}"] = {
            "Accuracy": accuracy,
            "Precision": precision,
            "Recall": recall,
            "F1-Score": f1,
    ...
    if best_score == 0:
        ...
        print(
            "- Data size is too small, resulting in one class dominating or missing, leading to the above issues. It's suggested to acquire a proper dataset."
        )

    return best_model, evaluation_metrics


best_model, evaluation_metrics = func(models, X_test, y_test, weights)
