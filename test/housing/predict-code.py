    ...
    def predict(preprocessing_dict, new_df):
        ...
        for step, parameters in preprocessing_dict.items():
            ...
            if step == "mean_imputation":
                ...
                non_pca_columns = [
                    col for col in new_df.columns if col not in columns_used
                ]
                ...
                for col, info in binning_info.items():
                    ...
                    if (
                        new_df[col].min() < bins_range[0][0]
                        or new_df[col].max() > bins_range[-1][1]
                    ):
                        print(
                            f"Warning: Value(s) in column '{col}' are out of bin range."
                        )
                        print("Execution stopped")
                        break

                    new_df[f"{col}_grouped_{bin_size}_bins"] = pd.cut(
                        new_df[col],
                        bins=[start for start, end in bins_range] + [info["max"] + 1],
                        labels=bin_labels,
                        include_lowest=True,
                    )
                ...
                if y_column and new_df[y_column].isnull().all():
                    X_columns = preprocessing_dict["impute_missing_values_with_rfg"][
                        "X_train_flow"
                    ]
                    ...
                    new_df.loc[new_df[y_column].isnull(), y_column] = (
                        y_missing_predicted
                    )
                    for col, encoder in encoders.items():
                        if col in new_df.columns:
                ...
                y_column = parameters.get("column_y", None)
                if y_column and new_df[y_column].isnull().all():
                    X_columns = preprocessing_dict["impute_missing_values_with_rfc"][
                        "X_train_flow"
                    ]
                    X_missing = new_df[new_df[y_column].isnull()][X_columns]
                ...
                categorical_info = preprocessing_dict.get(
                    "iterative_imputation", {}
                ).get("categorical", None)
                numerical_info = preprocessing_dict.get("iterative_imputation", {}).get(
                    "numerical", None
                )
                ...
                if categorical_info:
                    ...
                    for col in categorical_columns:
                        ...
                        col_values = [
                            x
                            for x in col_values
                            if not (isinstance(x, (int, float)) and np.isnan(x))
                            and x is not None
                        ]
                ...
                if numerical_info:
                    ...
                    if numerical_imputer:
                        new_df[numerical_columns] = numerical_imputer.transform(
                ...
                if categorical_info:
                    ...
                    if categorical_imputer:
                        new_df[categorical_columns] = categorical_imputer.transform(
            ...
            #     new_df = row_deletion_using_colvalues(*input_params)
                ...
                for column, encoder in label_encoders.items():
                    if column in new_df.columns:
                        new_df[column + "_encoded"] = encoder.transform(
                            new_df[[column]]
                        )
                ...
                df_with_scaled = pd.DataFrame(
                    scaled_data, columns=new_column_names, index=new_df.index
                )
