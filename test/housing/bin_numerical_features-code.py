def func():
    ...
    def bin_numerical_features(df, bins, columns):
        ...
        for col, bin_size in zip(columns, bins):
            ...
            bins_range = [(min_val + i*bin_size, min_val + (i+1)*bin_size) for i in range(num_bins)]
            bin_labels = [f"{start}-{end}" for start, end in bins_range]
            df[f"{col}_grouped_{bin_size}_bins"] = pd.cut(df[col], bins=[start for start, end in bins_range] + [max_val+1], labels=bin_labels, include_lowest=True)
            binning_info[col] = {
                'min': min_val,
                'max': max_val,
                'num_bins': num_bins,
